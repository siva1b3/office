...
# -t specifies the image name and tag (python-div-01-dep:latest)
# -f specifies the Dockerfile to be used for the build (Dockerfile.deps)
# The build context is set to the current directory (.)
docker build -t python-div-01-dep:latest -f Dockerfile.deps .  # <-- Add the build context (.)
...
if [ $? -eq 0 ]; then
    ...
    echo "✅ Docker image 'python-div-01-dep:latest' built successfully!"
else
